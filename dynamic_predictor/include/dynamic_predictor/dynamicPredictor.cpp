...
namespace dynamicPredictor{
    ...
    void predictor::predict(){ 
        // The onboard_detector version in this workspace exposes only current dynamic boxes.
        // Convert them to a single-sample history so the existing predictor pipeline still runs.
        this->posHist_.clear();
        this->velHist_.clear();
        this->accHist_.clear();
        this->sizeHist_.clear();

        std::vector<onboardDetector::box3D> dynamicBoxes;
        if (this->useFakeDetector_ and this->detectorGTReady_ and this->mapReady_){
            this->detectorGT_->getObstacles(dynamicBoxes);
        }
        else if (not this->useFakeDetector_ and this->detectorReady_ and this->mapReady_){
            this->detector_->getDynamicObstacles(dynamicBoxes, this->robotSize_);
        }

        if (!dynamicBoxes.empty()){
            this->posHist_.resize(dynamicBoxes.size());
            this->velHist_.resize(dynamicBoxes.size());
            this->accHist_.resize(dynamicBoxes.size());
            this->sizeHist_.resize(dynamicBoxes.size());
            for (size_t i = 0; i < dynamicBoxes.size(); ++i){
                const onboardDetector::box3D& box = dynamicBoxes[i];
                this->posHist_[i].push_back(Eigen::Vector3d(box.x, box.y, box.z));
                this->velHist_[i].push_back(Eigen::Vector3d(box.Vx, box.Vy, 0.0));
                this->accHist_[i].push_back(Eigen::Vector3d(box.Ax, box.Ay, 0.0));
                this->sizeHist_[i].push_back(Eigen::Vector3d(box.x_width, box.y_width, box.z_width));
            }
        }
