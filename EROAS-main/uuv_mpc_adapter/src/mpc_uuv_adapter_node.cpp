...
class UuvMpcAdapterNode {
 ...
 private:
  ...
  void logModeHeartbeat(const ros::Time& now,
    ...
    ROS_INFO_THROTTLE(
        3.0,
        "uuv_mpc_adapter state[GLOBAL_PASS=%d LOCAL_MPC=%d REJOIN_HOLD=%d] mode=%s obs=%zu "
  ...
  void planCB(const ros::TimerEvent&) {
    ...
    logModeHeartbeat(now, relevant_obs.size(), min_clearance_xy, dist_to_global_path_xy);
