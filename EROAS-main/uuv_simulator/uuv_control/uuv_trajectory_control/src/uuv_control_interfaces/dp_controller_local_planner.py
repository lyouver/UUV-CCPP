...
class DPControllerLocalPlanner(object):
    ...
    * `go_to_incremental` (*type:* `uuv_control_msgs.GoToIncremental`)
    ...
    def _update_trajectory_info(self):
        ...
        # Keep frame_id valid even when not using waypoint mode, otherwise
        # trajectory_marker_publisher may assert on empty frame_id.
    ...
    def _update_trajectory_from_msg(self, msg):
        ...
        if not self._traj_interpolator.init_from_trajectory_message(msg):
            self._logger.error('Failed to parse input trajectory message')
            return
        # External trajectory topic should immediately engage trajectory tracking.
    ...
    def start_waypoint_list(self, request):
        ...
        * `request` (*type:* `uuv_control_msgs.InitWaypointSet`)
        ...
        try:
            ...
            interp_method = request.interpolator.data if len(request.interpolator.data) > 0 else 'cubic'
            if interp_method not in self._traj_interpolator.get_interpolator_tags():
                self._logger.warning('Interpolator <%s> not available, fallback to cubic', interp_method)
                interp_method = 'cubic'
            ...
            with self._lock:
                ...
                waypointset_msg.header.stamp = rospy.Time.now()
                waypointset_msg.header.frame_id = self.inertial_frame_id
                if request.start_now:
                    waypointset_msg.start_time.data = rospy.Time.now()
                else:
                    waypointset_msg.start_time.data = t
                ...
                if wp_set.is_empty:
                    self._logger.error('Waypoint set is empty after workspace constraints')
                    return InitWaypointSetResponse(False)

                self._traj_interpolator.set_interp_method(interp_method)
                if self._traj_interpolator.set_waypoints(wp_set, self.get_vehicle_rot()):
                    ...
                    self.set_trajectory_running(True)
                    self._idle_circle_center = None
                    # For frequently-updated waypoint commands (e.g. MPC),
                    # restarting smooth-approach every update can cause
                    # heading hunting/spinning. Keep it off when start_now=True.
                    self._smooth_approach_on = (not request.start_now)
                    self._logger.info('============================')
                    self._logger.info('      WAYPOINT SET          ')
                    self._logger.info('============================')
                    self._logger.info('Interpolator = ' + interp_method)
            ...
            self._logger.error('start_waypoint_list exception: %s', str(ex))
