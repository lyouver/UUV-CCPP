...
namespace mapManager{
	...
	void occMap::initParam(){
		...
		// collision service option: unknown as occupied
		if (not this->nh_.getParam(this->ns_ + "/collision_unknown_as_occupied", this->collisionUnknownAsOccupied_)){
			this->collisionUnknownAsOccupied_ = true;
			cout << this->hint_ << ": No collision_unknown_as_occupied. Use default: true." << endl;
		}
		else{
			cout << this->hint_ << ": collision_unknown_as_occupied: " << this->collisionUnknownAsOccupied_ << endl;
		}

		// collision service option: neighborhood check radius
		if (not this->nh_.getParam(this->ns_ + "/collision_check_radius", this->collisionCheckRadius_)){
			this->collisionCheckRadius_ = 0.6;
			cout << this->hint_ << ": No collision_check_radius. Use default: 0.6 m." << endl;
		}
		else{
			cout << this->hint_ << ": collision_check_radius: " << this->collisionCheckRadius_ << endl;
		}
	...
	void occMap::initPrebuiltMap(){
		if (this->prebuiltMapDir_.empty() ||
			this->prebuiltMapDir_ == "No" ||
			this->prebuiltMapDir_ == "NO" ||
			this->prebuiltMapDir_ == "no" ||
			this->prebuiltMapDir_ == "None" ||
			this->prebuiltMapDir_ == "NONE" ||
			this->prebuiltMapDir_ == "none") {
			cout << this->hint_ << ": No prebuilt map configured, skip PCD loading." << endl;
			return;
		}
	...
	bool occMap::checkCollision(map_manager::CheckPosCollision::Request& req, map_manager::CheckPosCollision::Response& res){
		Eigen::Vector3d pos (req.x, req.y, req.z);
		Eigen::Vector3i idxCenter;
		this->posToIndex(pos, idxCenter);

		bool occupied = false;
		if (req.inflated){
			occupied = this->isInflatedOccupied(pos);
		}
		else{
			occupied = this->isOccupied(pos);
		}
		if ((not occupied) and this->collisionUnknownAsOccupied_){
			occupied = this->isUnknown(pos);
		}

		// Extra neighborhood query improves robustness for thin-surface observations:
		// a single voxel at the obstacle center can be free/unknown even when nearby
		// occupied voxels already exist.
		if ((not occupied) and this->collisionCheckRadius_ > 1e-6 and this->isInMap(idxCenter)){
			int nVox = std::max(1, int(std::ceil(this->collisionCheckRadius_ / this->mapRes_)));
			const double voxelDist2 = this->mapRes_ * this->mapRes_;
			const double radius2 = this->collisionCheckRadius_ * this->collisionCheckRadius_;
			for (int ix=-nVox; ix<=nVox and (not occupied); ++ix){
				for (int iy=-nVox; iy<=nVox and (not occupied); ++iy){
					for (int iz=-nVox; iz<=nVox and (not occupied); ++iz){
						double d2 = (ix * ix + iy * iy + iz * iz) * voxelDist2;
						if (d2 > radius2){
							continue;
						}
						Eigen::Vector3i idx = idxCenter + Eigen::Vector3i(ix, iy, iz);
						if (not this->isInMap(idx)){
							continue;
						}
						if (req.inflated){
							occupied = this->isInflatedOccupied(idx);
						}
						else{
							occupied = this->isOccupied(idx);
						}
						if ((not occupied) and this->collisionUnknownAsOccupied_){
							occupied = this->isUnknown(idx);
						}
					}
				}
			}
